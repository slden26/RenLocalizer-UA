...
class TLParser:
    ...
    @staticmethod
    def make_translation_id(file_path: str, line_number: int, text: str, context_path: Optional[List[str]] = None) -> str:
        """Deterministik ve context-aware translation ID (hash tabanlı)."""
        ctx = "|".join(context_path or [])
        base = f"{Path(file_path).as_posix()}|{line_number}|{ctx}|{text}"
