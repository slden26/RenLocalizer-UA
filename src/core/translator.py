...
class GoogleTranslator(BaseTranslator):
    ...
    async def translate_single(self, request: TranslationRequest) -> TranslationResult:
        ...
        if self.enable_lingva_fallback:
            ...
            if lingva_result:
                return TranslationResult(
                    request.text, lingva_result, request.source_lang, request.target_lang,
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if data2 and isinstance(data2, list) and data2[0]:
                    ...
                    return TranslationResult(
                        request.text, text, request.source_lang, request.target_lang,
    ...
    # Separator for batch translation - must NOT contain translatable words!
    # Using symbols/numbers that Google won't translate
    BATCH_SEPARATOR = "\n@@@000@@@\n"
    
    async def _multi_q(self, batch: List[TranslationRequest]) -> List[TranslationResult]:
        """Batch translation using separator method.
        
        Combines multiple texts with a unique separator, translates as one,
        then splits the result back into individual translations.
        """
        if not batch: return []
        if len(batch) == 1: return [await self.translate_single(batch[0])]
        
        self.logger.debug(f"Batch-sep: {len(batch)} texts, total chars: {sum(len(r.text) for r in batch)}")
        ...
        # All Google endpoints failed, try Lingva fallback (if enabled)
        if self.enable_lingva_fallback:
            self.logger.info(f"Batch: Google failed, trying Lingva fallback for {len(batch)} texts...")
            lingva_results = []
            for i, req in enumerate(batch):
                result = await self._translate_via_lingva(req.text, req.source_lang, req.target_lang)
                if result:
                    lingva_results.append(TranslationResult(
                        req.text, result, req.source_lang, req.target_lang,
                        TranslationEngine.GOOGLE, True, confidence=0.85
                    ))
                else:
                    lingva_results.append(TranslationResult(
                        req.text, "", req.source_lang, req.target_lang,
                        TranslationEngine.GOOGLE, False, "All endpoints failed"
                    ))
                # Log progress every 10 texts
                if (i + 1) % 10 == 0:
                    self.logger.debug(f"Lingva progress: {i+1}/{len(batch)}")
            return lingva_results
        
        # Final fallback to individual translations
        self.logger.warning(f"Batch: All methods failed, falling back to individual translations for {len(batch)} texts")
