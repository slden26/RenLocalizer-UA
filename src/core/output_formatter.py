...
class RenPyOutputFormatter:
    ...
    SKIP_FILE_EXTENSIONS = (
        ...
        '.py', '.json', '.txt', '.xml', '.csv',  # Data files
    )
    ...
    def _should_skip_translation(self, text: str) -> bool:
        ...
        # Skip URLs
        if re.match(r'^(https?://|ftp://|mailto:|www\.)', text_lower):
            return True
        ...
        # Skip save identifiers like "GameName-1234567890"
        ...
        stripped_of_tags = re.sub(r'\{[^}]*\}', '', text_strip)
        stripped_of_vars = re.sub(r'\[[^\]]*\]', '', stripped_of_tags)
    ...
    def escape_renpy_string(self, text: str) -> str:
        ...
        # CRITICAL: Escape newlines to prevent multiline string errors in Ren'Py
...
# Generated by: RenLocalizer v2.1.0
