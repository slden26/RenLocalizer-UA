...
def _classify_context(stack: List[Dict[str, object]]) -> List[str]:
    ...
    for item in stack:
        ...
        elif item.get('type') == 'nvl':
            path.append("nvl")
        elif item.get('type') == 'translate':
            path.append("translate")  # Will be used to skip content
        else:
            path.append(item.get('type'))
    return path


def _determine_text_type(ctx: List[str], stripped_line: str) -> str:
    """Determine text_type based on context and line content. (v2.4.1)"""
    # NVL mode dialogues
    if any('nvl' in c for c in ctx):
        return 'nvl_dialogue'
    # Menu choices
    if any('menu' in c for c in ctx):
        return 'menu'
    # Screen UI elements
    if any('screen' in c for c in ctx):
        return 'ui'
    # Python blocks
    if any('python' in c for c in ctx):
        return 'python_translatable'
    # Default: dialogue if in label or no context
    if not ctx or any('label' in c for c in ctx):
        return 'dialogue'
    return 'ui'
