...
class DiagnosticReport:
    ...
    def mark_translated(self, file_path: str, translation_id: str, translated_text: str):
        fr = self.files.get(file_path)
        if not fr:
            fr = FileReport(file_path=file_path)
            self.files[file_path] = fr
        fr.translated += 1
        fr.entries.append({'translation_id': translation_id, 'translated_text': translated_text, 'status': 'translated'})
    ...
    def mark_unchanged(self, file_path: str, translation_id: str):
        fr = self.files.get(file_path)
        if not fr:
            fr = FileReport(file_path=file_path)
            self.files[file_path] = fr
        fr.unchanged += 1
        fr.entries.append({'translation_id': translation_id, 'status': 'unchanged'})
