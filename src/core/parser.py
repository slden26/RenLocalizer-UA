...
class RenPyParser:
    def __init__(self, config_manager=None):
        ...
        self.python_block_re = re.compile(r'^(?:init\s+)?python\b.*:')
    ...
    def restore_placeholders(self, translated_text: str, placeholder_map: dict) -> str:
        ...
        for placeholder_id, original_placeholder in placeholder_map.items():
            corrupted_patterns = [
                placeholder_id.lower(),
                placeholder_id.upper(),
                placeholder_id.capitalize(),
                placeholder_id.replace('XYZ', 'Xyz'),
                placeholder_id.replace('XYZ', 'xyz'),
                placeholder_id.replace('XYZ', 'XYZ '),
                placeholder_id.replace('XYZ', ' XYZ'),
                placeholder_id.replace('XYZ', ' XYZ '),
                placeholder_id.replace('XYZ', 'Xyz '),
                placeholder_id.replace('XYZ', 'xyz '),
                placeholder_id.replace('XYZ', 'X Y Z'),
                placeholder_id.replace('XYZ', 'x y z'),
            ]
        ...
        for placeholder_id, original_placeholder in placeholder_map.items():
            if not placeholder_id.startswith('XYZ'):
                continue
            number_part = placeholder_id[3:]
            patterns = [
                rf'\b\s*[Xx][Yy][Zz]\s*{number_part}\s*\b',
                rf'\b\s*[Xx]\s*[Yy]\s*[Zz]\s*{number_part}\s*\b',
                rf'\b(?:XYZ|Xyz|xyz|X Y Z|x y z)\s*{number_part}\b',
                rf'\b[A-Za-z]{{3}}\s*{number_part}\b',
            ]
