    ...
    def _run_pipeline(self) -> PipelineResult:
        ...
        for tl_file in tl_files:
            ...
            if fp_norm.startswith(target_tl_dir):
                ...
                self.log_message.emit("info", f"Ba?ka dil klas?r? atland?: {tl_file.file_path}")
        ...
        try:
            ...
            if language_code:
                ...
                if len(subdirs) == 1:
                    ...
                    self.log_message.emit("info", f"tl dizininden otomatik dil secildi: {language_code}")
                else:
                    language_code = 'turkish'
                    self.log_message.emit("warning", "Hedef dil belirlenemedi; 'turkish' kullaniliyor")
            ...
            try:
                for existing in Path(game_dir).glob("a0_*_language.rpy"):
                    if existing.name != f"a0_{language_code}_language.rpy":
                        ...
                        self.log_message.emit("info", f"Eski dil baslaticisi silindi: {existing.name}")
            except Exception:
                pass
