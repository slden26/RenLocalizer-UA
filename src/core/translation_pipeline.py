        ...
        if needs_extraction or needs_decompile:
            ...
            if not success:
                ...
                if _os.name != "nt" and has_rpyc:
                    self.log_message.emit("warning", "RPA extraction failed, but RPYC files found. Continuing with RPYC Reader...")
                else:
                    return PipelineResult(
                        success=False,
    ...
    def _run_unren(self, project_path: str) -> bool:
        ...
        try:
            ...
            if not adapter.is_available():
                self.log_message.emit("error", "unrpa kütüphanesi yüklü değil. Lütfen 'pip install unrpa' çalıştırın.")
                return False
            ...
            self.log_message.emit("info", f"RPA arşivleri açılıyor: {game_dir}")
            ...
            try:
                ...
                if success:
                    ...
                    self.log_message.emit("info", "RPA dosyası bulunamadı veya zaten çıkarılmış.")
                    # rpyc dosyaları varsa devam et
                    if self._has_rpyc_files(str(game_dir)):
                        self.log_message.emit("info", "RPYC dosyaları mevcut, devam ediliyor...")
                        return True
                    return False
                    
            except Exception as e:
                self.log_message.emit("error", f"RPA çıkarma hatası: {e}")
                # Son şans - rpyc dosyaları varsa devam et
                if self._has_rpyc_files(str(game_dir)):
                    self.log_message.emit("info", "Hata olmasına rağmen RPYC dosyaları bulundu, devam ediliyor...")
                    return True
                return False
