...
class TranslationPipeline(QObject):
    ...
    def __init__(
        ...
        self.error_log_path = Path("pipeline_errors.log")
    ...
    def _log_error(self, message: str):
        """Persist errors for later inspection."""
        try:
                    ...
                    for t in texts:
                        ...
                        entry = TranslationEntry(
                            ...
                            translation_id=TLParser.make_translation_id(
                                str(rpymc_path), t.get('line_number', 0) or 0, text_val, ctx_path
                            )
                        )
            ...
            for i in range(0, total, batch_size):
                ...
                for entry in batch:
                    translation_id = getattr(entry, 'translation_id', '') or TLParser.make_translation_id(
                        entry.file_path, entry.line_number, entry.original_text, getattr(entry, 'context_path', [])
                    )
