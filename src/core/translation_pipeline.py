...
class TranslationPipeline(QObject):
    ...
    def _run_pipeline(self) -> PipelineResult:
        ...
        if getattr(self, 'include_deep_scan', False):
            self.log_message.emit("info", "Deep Scan (RPYC/Python) çalıştırılıyor...")
            try:
                parser = RenPyParser()
                # Scan source files
                ...
                if missing:
                     self.log_message.emit("info", f"Deep Scan: {len(missing)} adet eksik metin bulundu.")
                ...
                self.log_message.emit("warning", f"Deep Scan hatası: {e}")
