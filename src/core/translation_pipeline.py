...
class TranslationPipeline(QObject):
    ...
    def _run_pipeline(self) -> PipelineResult:
        ...
        if rpymc_files:
            ...
            self.log_message.emit('info', f"[DEBUG] .rpymc entry count: {len(self.rpymc_entries)}")
        ...
        try:
            normalized = self._normalize_tl_encodings(os.path.join(tl_path, self.target_language))
            if normalized:
        ...
        # --- .rpymc entry'lerini source_texts ve all_entries'ye ekle ---
        if getattr(self, 'rpymc_entries', None):
            self.log_message.emit('info', f"[DEBUG] .rpymc entry'leri ekleniyor: {len(self.rpymc_entries)} adet")
            for entry in self.rpymc_entries:
                if 'source_texts' in locals():
                    source_texts.append(entry)
                if 'all_entries' in locals():
                    all_entries.append(entry)
            # Eklenen ilk 3 rpymc entry'yi logla
            for i, entry in enumerate(self.rpymc_entries[:3]):
                self.log_message.emit('info', f"[DEBUG] .rpymc entry {i+1}: {entry}")
            self.log_message.emit('info', f"[DEBUG] source_texts uzunluğu: {len(source_texts) if 'source_texts' in locals() else 'N/A'}")
            self.log_message.emit('info', f"[DEBUG] all_entries uzunluğu: {len(all_entries) if 'all_entries' in locals() else 'N/A'}")
        ...
        try:
            ...
            self.log_message.emit('info', f"Diagnostic report yazıldı: {diag_path}")
        except Exception:
            pass
    ...
    def _normalize_tl_encodings(self, tl_dir: str) -> int:
        ...
        for file_path in tl_path.rglob("*.rpy"):
            try:
                ...
                self.log_message.emit("warning", f"Encoding normalize başarısız ({file_path}): {e}")
        return normalized
    ...
    def _create_language_init_file(self, game_dir: str):
        ...
        """
        try:
            # Hedef dil kodunu kullanarak dosya adı ve içerik oluştur
            language_code = getattr(self, 'target_language', 'turkish')
            # Dosya adı: a0_[lang]_language.rpy (RenPy dokümantasyonuna uygun, öne çıkar)
            ...
            # Sade ve dinamik başlatıcı içerik
    ...
    def _generate_all_strings_file(self, entries: List[dict], game_dir: str) -> str:
        ...
        if skipped:
            try:
                self.log_message.emit("debug", f"strings.rpy oluşturulurken teknik {skipped} giriş atlandı")
            except Exception:
                pass
    ...
    def _translate_entries(self, entries: List[TranslationEntry]) -> Dict[str, str]:
        ...
        if skipped:
            self.log_message.emit("debug", f"Teknik/yer tutucu {skipped} giriş çeviri dışı bırakıldı")
        ...
        try:
            ...
            for i in range(0, total, batch_size):
                ...
                for idx, result in enumerate(results):
                    ...
                    restored = restore_renpy_syntax(result.translated_text, placeholders) if result.translated_text else ""
                    ...
                    if not self.validate_placeholders(original=batch[idx].original_text, translated=restored):
                        self.log_message.emit("warning", f"PLACEHOLDER BOZULDU: {batch[idx].original_text} -> {restored}")
                    ...
                    if result.success and restored:
                        ...
                        try:
                            ...
                            if restored == batch[idx].original_text:
                                self.diagnostic_report.mark_unchanged(file_path, tid)
                            else:
                                self.diagnostic_report.mark_translated(file_path, tid, restored)
                        except Exception:
                            pass
                        if restored == batch[idx].original_text:
                            unchanged_count += 1
