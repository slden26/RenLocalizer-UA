...
class InfoDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.create_text_types_tab(dialog_data)
...
{formats_data.get('old_new', {}).get('example', '')}
    ...
    def create_text_types_tab(self, dialog_data):
        """Create text types information tab."""
        tt_data = dialog_data.get("text_types", {})
        if not tt_data:
            return
        
        tab_title = dialog_data.get("tabs", {}).get("text_types", "Text Types")
        ...
        core_items = ""
        for item in tt_data.get("core_types", {}).get("items", []):
            core_items += f"""
            <div style="border: 1px solid #3b82f6; border-radius: 6px; padding: 12px; margin-bottom: 10px; background-color: #1e3a5f;">
                <b style="color: #93c5fd; font-size: 14px;">💬 {item.get('name', '')}</b>
                <p style="margin: 8px 0 5px 0; color: #e5e7eb;">{item.get('description', '')}</p>
                <code style="background-color: #374151; color: #fcd34d; padding: 4px 8px; border-radius: 4px; font-size: 12px;">{item.get('example', '')}</code>
            </div>
            """
        
        # Build UI types section
        ui_items = ""
        for item in tt_data.get("ui_types", {}).get("items", []):
            ui_items += f"""
            <div style="border: 1px solid #10b981; border-radius: 6px; padding: 12px; margin-bottom: 10px; background-color: #065f46;">
                <b style="color: #6ee7b7; font-size: 14px;">🖥️ {item.get('name', '')}</b>
                <p style="margin: 8px 0 5px 0; color: #e5e7eb;">{item.get('description', '')}</p>
                <code style="background-color: #374151; color: #fcd34d; padding: 4px 8px; border-radius: 4px; font-size: 12px;">{item.get('example', '')}</code>
            </div>
            """
        ...
        notif_items = ""
        for item in tt_data.get("notification_types", {}).get("items", []):
            notif_items += f"""
            <div style="border: 1px solid #f59e0b; border-radius: 6px; padding: 12px; margin-bottom: 10px; background-color: #451a03;">
                <b style="color: #fcd34d; font-size: 14px;">🔔 {item.get('name', '')}</b>
                <p style="margin: 8px 0 5px 0; color: #e5e7eb;">{item.get('description', '')}</p>
                <code style="background-color: #374151; color: #fcd34d; padding: 4px 8px; border-radius: 4px; font-size: 12px;">{item.get('example', '')}</code>
            </div>
            """
        
        # Build config types section
        config_items = ""
        for item in tt_data.get("config_types", {}).get("items", []):
            config_items += f"""
            <div style="border: 1px solid #8b5cf6; border-radius: 6px; padding: 12px; margin-bottom: 10px; background-color: #2e1065;">
                <b style="color: #c4b5fd; font-size: 14px;">⚙️ {item.get('name', '')}</b>
                <p style="margin: 8px 0 5px 0; color: #e5e7eb;">{item.get('description', '')}</p>
                <code style="background-color: #374151; color: #fcd34d; padding: 4px 8px; border-radius: 4px; font-size: 12px;">{item.get('example', '')}</code>
            </div>
            """
        ...
        tips_items = "".join(
            f"<li style='margin-bottom: 6px; color: #a7f3d0;'>{tip}</li>"
            for tip in tt_data.get("tips", {}).get("items", [])
        )
        
        html_content = f"""
        <div style="font-family: Arial, sans-serif; line-height: 1.45; color: #e5e7eb;">
            <h2 style="color: #60a5fa; margin-bottom: 10px;">🔤 {tt_data.get('title', 'Text Type Filters')}</h2>
            <p style="margin-bottom: 20px; color: #d1d5db;">{tt_data.get('description', '')}</p>
            
            <!-- Core Types -->
            <div style="border: 2px solid #3b82f6; border-radius: 8px; padding: 15px; margin-bottom: 15px; background-color: #1e3a8a;">
                <h3 style="color: #93c5fd; margin-top: 0;">📝 {tt_data.get('core_types', {}).get('title', 'Core Text Types')}</h3>
                {core_items}
            </div>
            
            <!-- UI Types -->
            <div style="border: 2px solid #10b981; border-radius: 8px; padding: 15px; margin-bottom: 15px; background-color: #065f46;">
                <h3 style="color: #6ee7b7; margin-top: 0;">🖼️ {tt_data.get('ui_types', {}).get('title', 'Interface Texts')}</h3>
                {ui_items}
            </div>
            
            <!-- Notification Types -->
            <div style="border: 2px solid #f59e0b; border-radius: 8px; padding: 15px; margin-bottom: 15px; background-color: #451a03;">
                <h3 style="color: #fcd34d; margin-top: 0;">🔔 {tt_data.get('notification_types', {}).get('title', 'Notifications')}</h3>
                {notif_items}
            </div>
            
            <!-- Config Types -->
            <div style="border: 2px solid #8b5cf6; border-radius: 8px; padding: 15px; margin-bottom: 15px; background-color: #2e1065;">
                <h3 style="color: #c4b5fd; margin-top: 0;">⚙️ {tt_data.get('config_types', {}).get('title', 'Configuration Texts')}</h3>
                {config_items}
            </div>
            
            <!-- Tips -->
            <div style="border-left: 4px solid #22c55e; padding: 12px 15px; background-color: #052e16; border-radius: 6px; margin-bottom: 15px;">
                <h4 style="color: #86efac; margin-top: 0;">💡 {tt_data.get('tips', {}).get('title', 'Tips')}</h4>
                <ul style="margin-left: 18px;">
                    {tips_items}
                </ul>
            </div>
            ...
            <!-- Note -->
            <div style="background-color: #1e3a5f; color: #bfdbfe; padding: 12px; border-radius: 6px; border-left: 4px solid #3b82f6;">
                <p style="margin: 0;"><b>📌</b> {tt_data.get('note', '')}</p>
            </div>
        </div>
        """
