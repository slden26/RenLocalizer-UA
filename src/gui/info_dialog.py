...
class InfoDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.create_new_tools_tab(dialog_data)  # v2.4.0 new tools
...
{formats_data.get('old_new', {}).get('example', '')}
    ...
    def create_new_tools_tab(self, dialog_data):
        """Create new tools (v2.4.0) information tab."""
        features_data = dialog_data.get("features", {})
        new_tools_data = features_data.get("new_tools_v240", {})
        
        if not new_tools_data:
            return  # No new tools data available
        
        tab_title = "🆕 " + new_tools_data.get("title", "New Tools (v2.4.0)")
        ...
        html_content = f"""
        <div style="font-family: Arial, sans-serif; line-height: 1.45; color: #e5e7eb;">
            <h2 style="color: #60a5fa; margin-bottom: 10px;">{new_tools_data.get('title', 'New Tools (v2.4.0)')}</h2>
            <p style="margin-bottom: 20px; color: #d1d5db;">{new_tools_data.get('description', '')}</p>
        """
        
        # Add each tool
        tool_colors = [
            ("#3b82f6", "#1e3a8a", "#93c5fd"),  # Blue
            ("#10b981", "#065f46", "#6ee7b7"),  # Green
            ("#8b5cf6", "#2e1065", "#c4b5fd"),  # Purple
            ("#f59e0b", "#451a03", "#fcd34d"),  # Orange
        ]
        
        for i, tool in enumerate(new_tools_data.get("tools", [])):
            border_color, bg_color, title_color = tool_colors[i % len(tool_colors)]
            
            features_html = ""
            for feature in tool.get("features", []):
                features_html += f"<li style='margin-bottom: 4px; color: #e5e7eb;'>{feature}</li>"
            
            html_content += f"""
            <div style="border: 2px solid {border_color}; border-radius: 8px; padding: 15px; margin-bottom: 15px; background-color: {bg_color};">
                <h3 style="color: {title_color}; margin-top: 0; margin-bottom: 10px;">{tool.get('name', '')}</h3>
                <p style="color: #d1d5db; margin-bottom: 10px;">{tool.get('description', '')}</p>
                <p style="color: #93c5fd; margin-bottom: 8px;"><b>📍 Location:</b> {tool.get('menu_path', '')}</p>
                <p style="color: #e5e7eb; margin-bottom: 5px;"><b>Features:</b></p>
                <ul style="margin-left: 20px; margin-top: 5px;">
                    {features_html}
                </ul>
                <p style="background-color: #374151; color: #fef3c7; padding: 8px; border-radius: 4px; margin-top: 10px; margin-bottom: 0;">
                    <b>💡 Usage:</b> {tool.get('usage', '')}
                </p>
            </div>
            """
        
        html_content += """
        <div style="border-left: 4px solid #22c55e; padding: 12px 15px; background-color: #052e16; border-radius: 6px; margin-top: 10px;">
            <h4 style="color: #86efac; margin-top: 0;">💡 Quick Tips</h4>
            <ul style="margin-left: 18px; color: #a7f3d0;">
                <li style="margin-bottom: 4px;">Run Health Check BEFORE translation to catch issues early</li>
                <li style="margin-bottom: 4px;">Use Pseudo-Localization to test UI without API costs</li>
                <li style="margin-bottom: 4px;">Font Check is especially important for CJK and Asian languages</li>
                <li style="margin-bottom: 4px;">Fuzzy Matching works best with CLI for batch operations</li>
            </ul>
        </div>
        </div>
        """
