...
class MainWindow(QMainWindow):
    ...
    def create_menu_bar(self):
        ...
        # New tools
        health_check_action = QAction("🔍 " + self.config_manager.get_ui_text("health_check_menu", "Health Check"), self)
        health_check_action.triggered.connect(self.show_health_check_dialog)
        tools_menu.addAction(health_check_action)
        
        font_check_action = QAction("🔤 " + self.config_manager.get_ui_text("font_check_menu", "Font Compatibility"), self)
        font_check_action.triggered.connect(self.show_font_check_dialog)
        tools_menu.addAction(font_check_action)
        
        pseudo_action = QAction("🧪 " + self.config_manager.get_ui_text("pseudo_menu", "Pseudo-Localization"), self)
        pseudo_action.triggered.connect(self.show_pseudo_dialog)
        tools_menu.addAction(pseudo_action)
        
        fuzzy_action = QAction("🔄 " + self.config_manager.get_ui_text("fuzzy_menu", "Smart Update (Fuzzy)"), self)
        fuzzy_action.triggered.connect(self.show_fuzzy_dialog)
        tools_menu.addAction(fuzzy_action)

        # TL çeviri menüsü (mevcut tl/<dil> klasörü için)
        ...
        patreon_action = QAction("❤️ " + self.config_manager.get_ui_text("support_patreon", "Support on Patreon"), self)
    ...
    def populate_engine_combo(self):
        ...
        engines = [
            ...
            (TranslationEngine.PSEUDO, "🧪 Pseudo-Localization (Test)"),
        ]
    ...
    def show_health_check_dialog(self):
        ...
        try:
            ...
            if report.is_healthy:
                QMessageBox.information(self, "✅ " + self.config_manager.get_ui_text("health_check_title", "Health Check"), result_text)
            else:
                QMessageBox.warning(self, "⚠️ " + self.config_manager.get_ui_text("health_check_title", "Health Check"), result_text)
    ...
    def show_font_check_dialog(self):
        ...
        try:
            ...
            if summary['incompatible_fonts'] > 0:
                QMessageBox.warning(self, "⚠️ " + self.config_manager.get_ui_text("font_check_title", "Font Check"), result_text)
            else:
                QMessageBox.information(self, "✅ " + self.config_manager.get_ui_text("font_check_title", "Font Check"), result_text)
    ...
    def show_pseudo_dialog(self):
        ...
        modes = ["both (expand + accent)", "expand ([!!! markers !!!])", "accent (àccéntéd)"]
        ...
        try:
            ...
            if not rpy_files:
                QMessageBox.warning(self, "⚠️ No Files", "No .rpy files found. Run UnRen first if game is compiled.")
                return
            ...
            QMessageBox.information(
                self, 
                "✅ " + self.config_manager.get_ui_text("pseudo_title", "Pseudo-Localization"),
                f"Pseudo-localization complete!\n\n"
                f"• Strings processed: {translated_count}\n"
                f"• Output: {output_dir}\n\n"
                f"To test in game:\n"
                f"1. In game preferences, select 'pseudo' as language\n"
                f"2. Look for [!!! markers !!!] and àccéntéd characters"
            )
    ...
    def show_fuzzy_dialog(self):
        ...
        self._add_log("info", f"Running fuzzy matching: {old_tl} → {new_tl}")
        ...
        try:
            ...
            if not old_entries:
                QMessageBox.warning(self, "⚠️ Warning", "No translations found in OLD folder.")
                return
            
            if not new_entries:
                QMessageBox.warning(self, "⚠️ Warning", "No entries found in NEW folder.")
                return
            ...
            if report.auto_apply_count > 0:
                result_text += f"\n\n✅ {report.auto_apply_count} translations can be auto-recovered!"
                
                # Ask if user wants to apply
                reply = QMessageBox.question(
                    self,
                    "🔄 " + self.config_manager.get_ui_text("fuzzy_title", "Smart Update"),
