...
class SettingsDialog(QDialog):
    ...
    def create_general_tab(self):
        ...
        self.check_updates_now_btn = QPushButton(self.config_manager.get_ui_text("check_updates_now_button"))
        self.check_updates_now_btn.setToolTip(self.config_manager.get_ui_text("check_updates_now_tooltip"))
        self.check_updates_now_btn.clicked.connect(self.check_updates_now_clicked)
        layout.addRow(self.config_manager.get_ui_text("check_updates_now_label"), self.check_updates_now_btn)
    ...
    def create_translation_tab(self):
        ...
        # Deep Scan settings (experimental)
        deep_scan_group = QGroupBox(self.config_manager.get_ui_text("deep_scan_group"))
        deep_scan_layout = QFormLayout(deep_scan_group)
        
        self.enable_deep_scan_check = QCheckBox()
        self.enable_deep_scan_check.setToolTip(self.config_manager.get_ui_text("enable_deep_scan_tooltip"))
        deep_scan_layout.addRow(self.config_manager.get_ui_text("enable_deep_scan_label"), self.enable_deep_scan_check)
        
        self.enable_rpyc_reader_check = QCheckBox()
        self.enable_rpyc_reader_check.setToolTip(self.config_manager.get_ui_text("enable_rpyc_reader_tooltip"))
        deep_scan_layout.addRow(self.config_manager.get_ui_text("enable_rpyc_reader_label"), self.enable_rpyc_reader_check)
        
        layout.addWidget(deep_scan_group)
    ...
    def load_settings(self):
        ...
        # Deep Scan settings (experimental)
        self.enable_deep_scan_check.setChecked(
            getattr(self.config_manager.translation_settings, 'enable_deep_scan', False)
        )
        self.enable_rpyc_reader_check.setChecked(
            getattr(self.config_manager.translation_settings, 'enable_rpyc_reader', False)
        )
    ...
    def save_settings(self):
        ...
        # Deep Scan settings (experimental)
        self.config_manager.translation_settings.enable_deep_scan = self.enable_deep_scan_check.isChecked()
        self.config_manager.translation_settings.enable_rpyc_reader = self.enable_rpyc_reader_check.isChecked()
    ...
    def check_updates_now_clicked(self):
        """Handle update check button click."""
        try:
            parent_window = self.parent()
            if hasattr(parent_window, 'check_for_updates'):
                parent_window.check_for_updates(show_no_update=True)
            else:
                from PyQt6.QtWidgets import QMessageBox
                QMessageBox.warning(
                    self,
