...
class TranslationWorker(QObject):
    ...
    def stop(self):
        ...
        self.is_running = False
        self.logger.info("Translation stop requested - setting flags")
    ...
    def run(self):
        ...
        try:
            ...
            try:
                self._loop.run_until_complete(self._translate_texts())
            except asyncio.CancelledError:
                self.logger.info("Translation was cancelled")
            finally:
                # Clean up pending tasks
                try:
                    if self._loop and self._loop.is_running():
                        self._loop.stop()
                    pending = asyncio.all_tasks(self._loop) if self._loop else set()
                    for task in pending:
                        task.cancel()
                    if pending and self._loop and not self._loop.is_closed():
                        self._loop.run_until_complete(asyncio.gather(*pending, return_exceptions=True))
                except Exception as e:
                    self.logger.debug(f"Cleanup exception (expected): {e}")
                finally:
            ...
            for i in range(0, len(requests), batch_size):
                ...
                for result in batch_results:
                    if result.success and result.translated_text:
                        ...
                        # Copy text_type from metadata to result
                        result.text_type = result.metadata.get('type', None)
