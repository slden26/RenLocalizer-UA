...
class ApiKeysDialog(QDialog):
    ...
    def create_bing_tab(self):
        """Create Microsoft Translator API tab."""
        widget = QGroupBox()
        layout = QVBoxLayout(widget)
        
        # Info
        info = QLabel(self.tr(
            "Microsoft Translator (Bing) provides good quality translations with competitive pricing."
        ))
        info.setWordWrap(True)
        layout.addWidget(info)
        ...
        self.bing_key_input.setPlaceholderText("Enter your Bing API key")
        form_layout.addRow(self.tr("API Key:"), self.bing_key_input)
        layout.addLayout(form_layout)
        
        # Show/Hide button
        show_button = QPushButton(self.tr("Show/Hide"))
        show_button.clicked.connect(lambda: self.toggle_password_visibility(self.bing_key_input))
        form_layout.addWidget(show_button)
        ...
        instructions.setReadOnly(True)
        instructions.setHtml(self.tr(
            "<b>How to get Microsoft Translator API key:</b><br>"
            "1. Go to <a href='https://azure.microsoft.com/'>Microsoft Azure</a><br>"
            "2. Create a free account or sign in<br>"
            "3. Create a new 'Translator' resource<br>"
            "4. Go to 'Keys and Endpoint'<br>"
            "5. Copy one of the API keys<br><br>"
            "<b>Free tier:</b> 2 million characters/month"
        ))
        layout.addWidget(instructions)
        
        self.tab_widget.addTab(widget, "Microsoft")
    
    def create_yandex_tab(self):
        """Create Yandex Translate API tab."""
        widget = QGroupBox()
        layout = QVBoxLayout(widget)
        
        # Info
        info = QLabel(self.tr(
            "Yandex Translate provides good translations, especially for Russian and Eastern European languages."
        ))
        info.setWordWrap(True)
        layout.addWidget(info)
        ...
        self.yandex_key_input.setEchoMode(QLineEdit.EchoMode.Password)
        self.yandex_key_input.setPlaceholderText("AQVNxxxxxxxxxx")
        form_layout.addRow(self.tr("API Key:"), self.yandex_key_input)
        layout.addLayout(form_layout)
        
        # Show/Hide button
        show_button = QPushButton(self.tr("Show/Hide"))
        show_button.clicked.connect(lambda: self.toggle_password_visibility(self.yandex_key_input))
        form_layout.addWidget(show_button)
        ...
        instructions.setReadOnly(True)
        instructions.setHtml(self.tr(
            "<b>How to get Yandex Translate API key:</b><br>"
            "1. Go to <a href='https://cloud.yandex.com/'>Yandex Cloud</a><br>"
            "2. Create an account and billing account<br>"
            "3. Create a new folder in your cloud<br>"
            "4. Enable the 'Yandex Translate' service<br>"
            "5. Create an API key in the service account<br><br>"
            "<b>Free tier:</b> First 1 million characters/month"
        ))
        layout.addWidget(instructions)
        
        self.tab_widget.addTab(widget, "Yandex")
    ...
    def load_api_keys(self):
        ...
        self.bing_key_input.setText(self.config_manager.get_api_key("bing"))
        self.yandex_key_input.setText(self.config_manager.get_api_key("yandex"))
    ...
    def save_api_keys(self):
        ...
        self.config_manager.set_api_key("bing", self.bing_key_input.text().strip())
        self.config_manager.set_api_key("yandex", self.yandex_key_input.text().strip())
