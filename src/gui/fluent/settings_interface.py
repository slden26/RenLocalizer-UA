...
class SettingsInterface(ScrollArea):
    ...
    def _create_general_group(self):
        ...
        self.theme_combo.addItem(self.config_manager.get_ui_text("theme_dark", "🌙 Koyu"), "dark")
        self.theme_combo.addItem(self.config_manager.get_ui_text("theme_light", "☀️ Açık"), "light")
        ...
        current_theme = getattr(self.config_manager.app_settings, 'app_theme', 'dark')
        ...
        self.format_card = SettingCard(
            icon=FIF.EDIT,
            title=self.config_manager.get_ui_text("output_format_label", "Çıktı Formatı"),
            content=self.config_manager.get_ui_text("output_format_desc", "Ren'Py çeviri dosyası formatı"),
            parent=self.general_group
        )
        self.format_combo = ComboBox(self.format_card)
        self.format_combo.addItem(self.config_manager.get_ui_text("output_format_official", "OLD_NEW (Official)"), "old_new")
        self.format_combo.addItem(self.config_manager.get_ui_text("output_format_modern", "SIMPLE (Modern)"), "simple")
        self.format_card.hBoxLayout.addWidget(self.format_combo, 0, Qt.AlignmentFlag.AlignRight)
        self.format_card.hBoxLayout.addSpacing(16)
        
        # Set current format
        current_format = self.config_manager.app_settings.output_format
        self.format_combo.setCurrentIndex(0 if current_format == "old_new" else 1)
        self.format_combo.currentIndexChanged.connect(
            lambda i: self._save_setting("output_format", self.format_combo.currentData())
        )
        self.general_group.addSettingCard(self.format_card)
    ...
    def _on_theme_changed(self, index: int):
        """Handle theme change - requires restart for consistent rendering."""
        ...
        # Show restart notification (theme requires restart to apply consistently)
        if self.parent_window:
            self.parent_window.show_info_bar(
                "info",
                self.config_manager.get_ui_text("theme_changed", "Tema Değiştirildi"),
                self.config_manager.get_ui_text("theme_restart", "Temanın tam olarak uygulanması için uygulamayı yeniden başlatın.")
            )
    ...
    def _on_format_changed(self, index: int):
        ...
        new_format = "old_new" if index == 0 else "simple"
        self.config_manager.app_settings.output_format = new_format
        self.config_manager.save_config()
