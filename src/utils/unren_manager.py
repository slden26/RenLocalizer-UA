...
class UnRenManager:
    ...
    def _extract_zip(self, zip_path: Path, destination: Path) -> None:
        ...
        try:
            ...
            # Some archives contain files inside subfolders; move everything up
    ...
    def run_unren(
        ...
        # Use cmd.exe /c for batch files to ensure proper stdin handling
        if automation_script is not None:
            command = ["cmd.exe", "/c", str(script_path), str(project_dir)]
        else:
