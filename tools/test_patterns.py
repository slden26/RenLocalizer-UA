...
test_lines = [
    ...
    ("text 'Price: $35' size 50 color '#FCD756'", True, 'single with colon and dollar'),
        # f-string and format tests
        ('text f"Hello {player_name}"', True, 'f-string in text'),
        ('renpy.notify(f"Score: {score}")', True, 'f-string in notify'),
        ('text "Hello {}".format(player_name)', True, '.format in text'),
        ('text "%s points" % points', True, 'percent-format in text'),
        ("text _('Already OK {player}')", False, 'Already wrapped with _() should NOT change'),
]
